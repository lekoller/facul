-- a)
DECLARE CURSOR one_dot_three_a_cursor IS
SELECT sac.nr_sac -- Número da ocorrência do SAC
,
...
FROM mc_sgv_sac sac
    JOIN mc_produto pro ON sac.cd_produto = pro.cd_produto
    JOIN mc_cliente cli ON sac.nr_cliente = cli.nr_cliente;
BEGIN FOR a_line IN one_dot_three_a_cursor LOOP dbms_output.put_line('Número da ocorrência do SAC: ' || a_line.nr_sac);
dbms_output.put_line(
    'Data de abertura do SAC: ' || a_line.dt_abertura_sac
);
dbms_output.put_line(
    'Hora de abertura do SAC: ' || a_line.hr_abertura_sac
);
...
dbms_output.put_line(
    'Valor unitário do produto: ' || a_line.vl_unitario
);
dbms_output.put_line(
    'Percentual do lucro unitário do produto: ' || a_line.vl_perc_lucro
);
...
DECLARE CURSOR one_dot_three_a_cursor IS
SELECT sac.nr_sac,
    -- Número da ocorrência do SAC
    sac.dt_abertura_sac,
    -- Data de abertura do SAC
    sac.hr_abertura_sac,
    -- Hora de abertura do SAC
    sac.tp_sac,
    -- Tipo do SAC
    pro.cd_produto,
    -- Código do produto
    ...
    pro.vl_perc_lucro,
    -- Percentual do lucro unitário do produto
    cli.nr_cliente,
    -- Número do cliente
    cli.nm_cliente -- Nome do cliente
FROM mc_sgv_sac sac
...
BEGIN FOR a_line IN one_dot_three_a_cursor LOOP -- Transform TP_SAC into DS_TIPO_CLASSIFICACAO_SAC
CASE
    a_line.tp_sac
    WHEN 'S' THEN v_tipo_classificacao_sac := 'SUGESTÃO';
WHEN 'D' THEN v_tipo_classificacao_sac := 'DÚVIDA';
WHEN 'E' THEN v_tipo_classificacao_sac := 'ELOGIO';
ELSE v_tipo_classificacao_sac := 'CLASSIFICAÇÃO INVÁLIDA';
END CASE
;
...
SELECT end_cli.sg_estado,
    end_cli.nm_estado INTO v_sg_estado,
    v_nm_estado
FROM mc_end_cli end_cli
    JOIN mc_logradouro logr ON end_cli.nr_endereco = logr.nr_endereco
    JOIN mc_bairro bairro ON logr.cd_bairro = bairro.cd_bairro
    JOIN mc_cidade cidade ON bairro.cd_cidade = cidade.cd_cidade
    JOIN mc_estado estado ON cidade.cd_estado = estado.cd_estado
WHERE end_cli.nr_cliente = a_line.nr_cliente;
-- Calculate VL_ICMS_PRODUTO
SELECT pf0110.fun_mc_gera_aliquota_media_icms_estado(v_sg_estado) INTO v_vl_icms_produto
FROM dual;
...
/ -- Inserting random data into mc_sgv_sac table
